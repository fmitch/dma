...
def plotNNFilter(units):
    ...
    dim = int(units.shape[1])
    plt.figure(1)
    n_columns = 8
    n_rows = int(filters / n_columns)
    grid = np.empty((n_rows*dim, n_columns*dim))
    for i in range(n_columns):
        for j in range(n_rows):
            grid[j*dim:(j+1)*dim,i*dim:(i+1)*dim] = units[0,:,:,i*n_columns + j]
    #plt.imshow(grid,  cmap="gray")
    #plt.show()
    imageio.imsave('filters.png', grid)
